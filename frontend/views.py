...
def index(request):
    data = requests.get('http://localhost:8000/api/all-projects/').json()
    return render(request,"frontend/index.html",{'data':data})

def project(request,pk):
    data = requests.get('http://localhost:8000/api/project/'+str(pk)).json()
    params ={'pid':pk}
    task_data = requests.get('http://localhost:8000/api/task/',params=params).json()
    return render(request,"frontend/project.html",{'data':data,'task_data':task_data})

def task(request,pk1,pk2):
    # params ={'pid':pk2}
    data = requests.get('http://localhost:8000/api/task/'+str(pk2)).json()
...
def task_edit(request,pk1,pk2):
    ...
    if request.method == "POST":    
        ...
        if(ed!=''):
            data['end_date'] = ed
        r = requests.put('http://localhost:8000/api/task/'+str(pk2)+"/",data=data)
...
def task_delete(request,pk1,pk2):
    data = requests.delete('http://localhost:8000/api/task/'+str(pk2))
...
def task_create(request,pk):
    ...
    if request.method == "POST":    
        ...
        if(ed!=''):
            data['end_date'] = ed
        r = requests.post('http://localhost:8000/api/task-add/',data=data)
...
def project_create(request):
    ...
    if request.method == "POST":    
        ...
        if(ed!=''):
            data['end_date'] = ed
        r = requests.post('http://localhost:8000/api/project/',data=data)
...
def project_edit(request,pk):
    ...
    if request.method == "POST":    
        ...
        if(ed!=''):
            data['end_date'] = ed
        r = requests.put('http://localhost:8000/api/project/'+str(pk)+"/",data=data)
        image = request.FILES.get('img',False)
        if(image):
            obj = Project.objects.filter(id=pk).first()
            obj.image = image
...
def project_delete(request,pk):
    data = requests.delete('http://localhost:8000/api/project/'+str(pk))
    return redirect('/')
